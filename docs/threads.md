...
graph LR
  ...
  F0 -->|every other write, truncate, dirty flush/fsync: per-handle lane, reply in the job, never refused| DK[[disk ×4, q∞]]
  UP -.->|refused: append only| DK
  F0 -->|released handle's staging delete/move| DK
...
| `disk` | 4 | unbounded | never refused: a lane submits one step at a time, so the queue holds at most one step per handle with a kernel request outstanding | every `write()` that does not graduate a chunk (pwrite/append, seeding the staging file from the kept copy on the first one), a truncate, `flush`/`fsync` of a dirty handle (reply in the job), a `release` queued behind a handle's in-flight write, and deleting or moving a released handle's staging file |
