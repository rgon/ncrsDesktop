...
## Fixed: inline staging write on fuser-0 (2026-09-25)

- **HIGH (latency): a write's staging-file I/O ran on fuser-0.** A latency investigation caught fuser-0 in D state (`jbd2` `do_get_write_access` ← `file_update_time`) during a slow host disk: the write class `Inline` still did its pwrite/append on the dispatch thread whenever the handle's lane was idle, which for a plain sequential writer is always, and a pwrite updates the mtime, which waits for the ext4 journal. Now every write is a lane step on `bg::DISK` (a graduation on `bg::UPLOAD`) owning its reply; so is every truncate and the flush/fsync of a dirty handle. Release keeps its in-memory commit on fuser-0 (so an open or rename the kernel sends after RELEASE finds it journaled) and hands the staging deletes and moves (never-written handle, failed stream, unlinked file) to `bg::DISK` after the reply, keeping the staging reservation until they ran. `WriteCost::Inline`/`Seed` became `Stage`, so `write_cost` no longer stats the staging file on fuser-0 either. What fuser-0 still does to the cache dir is read a staging file's length (release, getattr's overlay), which needs no journal.
- **Refusal policy:** `bg::DISK` never refuses (unbounded queue, like `mutate`). Every job on it is a lane step owning a kernel reply (or a released handle's cleanup), and a lane submits one step at a time, so the queue holds at most one step per handle with a request outstanding: bounded by the kernel, not by anything the daemon could shed. The two alternatives are worse: running the step on the caller is the disk wait this removes, and EAGAIN goes to the application (`cp` fails). A graduation `bg::UPLOAD` refuses spills to `bg::DISK` and only appends (to `mutate`, where it graduates, once the tail holds `TAIL_CAP_CHUNKS`); `fh_lane::Ran::OnSpill` tells the two apart. A step runs on its caller only if neither pool can start a thread at all.
- **Classification of queued writes.** With every write on a pool, overlapping writes (shared-mapping writeback) all queue before the first appends, and judged by `total_written` none of them would ever fill a chunk, so streaming stopped and the tail cap was defeated. `OpenFile::dispatched_end` tracks where the handle will be once the writes already dispatched have run, and `classify_write` judges against that.
- Tests: every write-path `Rig` dispatch runs under `write_path::as_dispatch_thread`, which panics on staging-file I/O on that thread (`the_dispatch_thread_marker_catches_staging_io` shows it trips); `the_disk_pool_never_refuses_so_a_stalled_disk_queues_writes_without_holding_the_dispatcher` (4,200 handles × 4 steps onto a pool whose one worker is stuck: dispatch returns, nothing answered, nothing refused, then every reply in request order per handle and every file right); `a_refused_graduation_only_appends_on_the_disk_pool_and_the_next_write_catches_up`; the existing ordering and reply-order tests unchanged.
- **Throughput cost of the hop** (bookworm release builds, walker image, rclone behind the fault proxy with chunked uploads off, the cache dir and the server's data on tmpfs so the host disk is out of the measurement; the host's load average was 12–37 from other work throughout, so the spread is wide). Medians of 6–12 runs, interleaved:

  | Workload | before (439edf6) | after (c51a3f8) | after + worker linger (1d33a44) |
  |---|---|---|---|
  | `dd bs=128k`, 1 GiB | 492 MB/s | 231 MB/s | 247 MB/s |
  | `dd bs=1M`, 1 GiB (1 MiB FUSE writes) | 382 MB/s | 288 MB/s | 293 MB/s |
  | 2,000 × 4 KiB files (create, write, close) | 132 files/s | 169 files/s | 135 files/s |

  In-process (`dispatch_write` and wait vs the same `write_answer` inline, staging on `/dev/shm`): 128 KiB writes 1.2–1.6 GB/s inline vs 0.47–0.54 GB/s through the pool without the linger and 0.63–0.89 GB/s with it; 4 KiB writes 64–110 k/s inline vs 6–9 k/s without the linger and 17–22 k/s with it. So the hop adds ~30–40 µs per write on this host (a futex wake and a context switch; the linger removed the thread start). It costs a single sequential writer on a RAM-backed cache about half its throughput at 128 KiB writes and a quarter at 1 MiB; small-file workloads, dominated by create/flush/release, do not see it, and on the disk-backed cache the first runs were dominated by the host disk (24–164 MB/s before, 22–90 MB/s after). Coalescing queued writes of a handle cannot help a sequential writer, which never has more than one write outstanding; the only way to hide the hop is to answer a write before its bytes are in the staging file (a daemon-side writeback cache), which would need reads, stats and errors to wait for or report the lane, and was not done. Several writers now stage in parallel on 4 `disk` workers instead of one after another on `fuser-0`. A spinning worker (poll the queue 50 µs before sleeping) was tried and dropped: no measurable gain on the loaded host, at the cost of burnt CPU.
- **Still on fuser-0:** `create()` and a truncating `open()` create their empty staging file there (`File::create`), outside the lane. Moving that means a staging file that may not exist yet at the first read, stat or release of the handle.

## Fixed: flaky `the_fast_path_stays_fast_while_slow_resolvers_wait_on_a_wide_stream`

It asserted the cache-hit path's p99 under 1 ms (release) or 10 ms (debug) while 16 resolvers waited on a 100k-entry stream, and failed under host load or parallel test runs (a debug run here measured p99 8.2 ms, max 54 ms, on a correct build). It now asserts what keeps the hit path fast, by count: `pending_find` records (test-only `PendingProbe`) how many times waiters took the cache lock to look in the stream and the most stream entries one look moved and indexed. One look must handle at most `PENDING_DRAIN_BATCH` entries (never the backlog, never a copy of the partial listing), and the looks must stay within one per waiter per 50 ms tick plus one per drained batch (a wake-up storm, which is what stalled the hit path, looks tens of thousands of times; a correct run looks ~1,100 times against a bound of ~1,400). Host load only lowers the count. The hits must still be answered inline, and the latencies are printed.
